...
const text = fs.readFileSync('src/components/Sidebar.jsx', 'utf8');

const target = '<div className="sidebar-section" style={{ marginTop: \'16px\', background: \'var(--bg-darker)\', padding: \'12px\', borderRadius: \'8px\' }}>\r\n        <label style={{ display: \'block\', marginBottom: \'8px\', fontWeight: \'bold\' }}>GIF Export (Wave Effect)</label>';
const targetUnix = '<div className="sidebar-section" style={{ marginTop: \'16px\', background: \'var(--bg-darker)\', padding: \'12px\', borderRadius: \'8px\' }}>\n        <label style={{ display: \'block\', marginBottom: \'8px\', fontWeight: \'bold\' }}>GIF Export (Wave Effect)</label>';

let idx = text.indexOf(target);
if (idx === -1) idx = text.indexOf(targetUnix);

if (idx !== -1) {
    const newUI = `      <div className="sidebar-section" style={{ marginTop: '24px' }}>
...
`;
    const finalStr = text.substring(0, idx) + newUI;
    fs.writeFileSync('src/components/Sidebar.jsx', finalStr, 'utf8');
    console.log("Done updating export UI");
} else {
    console.log("Not found target for export UI");
}
